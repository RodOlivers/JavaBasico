...
public class MenuPessoa {
	...
	//CRIA A OPÇÃO 1) CADASTRAR, 2)TODOS
	//AO CADASTRAR DEM QUE INFORMAR AS PERSONALIDADES E VERIFICAR SE ELAS JA EXISTE NO BANCO
	...
	// 3) Deletar
		...
		Integer opcao = 0;
			do {
				opcao = Leitor.lerValor("1) CADASTRAR, 2)TODOS\n", sc, 0);
				System.out.println("\n---------------------------\n");
	...
	private static void cadastrar(PersonalidadeDAO personalidadeDAO, PessoaDAO pessoaDAO, Scanner sc) {
		...
		p.setIdade(Leitor.lerValor("Qual idade?", sc, 0));
		Sexo sexoEnum = null;
		do {
		...
		do {
			Boolean querAdicionarPersonalidade = Leitor.lerValor("Deseja adicionar uma personalidade?", sc, false);
